document.addEventListener('DOMContentLoaded', () => {
  ...
  // Buscar lista de pacientes na API
  fetch('https://api.exemplo.com/pacientes') // Troque pela URL real da sua API
    .then(res => res.json())
    .then(pacientes => {
  ...
  // Ao selecionar um paciente, buscar histórico
  selectPaciente.addEventListener('change', () => {
    historicoContainer.innerHTML = '';
    const pacienteId = selectPaciente.value;
    if (!pacienteId) return;

    fetch(`https://api.exemplo.com/historico/${pacienteId}`) // Troque pela URL real da sua API
      .then(res => res.json())
      .then(historicos => {
